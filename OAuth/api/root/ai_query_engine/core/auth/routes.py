...
from root.ai_query_engine.core.auth.common.exceptions import (
...
from root.ai_query_engine.core.auth.core import auth_manager
from root.ai_query_engine.core.auth.common.middleware import (
    get_current_user,
...
from root.ai_query_engine.core.auth.session_management import token_cookie_manager
from root.ai_query_engine.core.auth.common.exceptions import *
from root.ai_query_engine.core.auth.common.constants import SUCCESS_LOGOUT
    ...
    try:
        ...
        if result.get("requires_otp"):
            return {
                "success": False,
                "message": result["message"],
                "data": {"requires_otp": True, "otp_status": result["otp_status"]},
            }
...
async def get_auth_status(
    ...
    if current_user:
        return {
            "success": True,
            "data": {"is_authenticated": True, "user": current_user.to_dict()},
        }
    else:
        return {"success": True, "data": {"is_authenticated": False}}
...
async def oauth_authorize(
    provider: str, social_request: SocialLoginRequest, request: Request
) -> Dict[str, Any]:
    """
...
async def oauth_callback(
    ...
    try:
        ...
        token_cookie_manager.set_auth_cookies(
            response, tokens.access_token, tokens.refresh_token, user_data
        )
...
async def health_check() -> HealthCheck:
    ...
    return HealthCheck(
        ...
        dependencies={
            ...
            "email": (
                "configured"
                if auth_manager.email_manager.smtp_configured
                else "not_configured"
            ),
        },
    )
