...
class OTPManager:
    ...
    async def verify_code(
        ...
        await self.rate_limiter.check_limit(
            f"otp_verify:{user_id}", config.RATE_LIMIT_OTP_PER_MIN, 60
        )
    ...
    def verify_backup_code(
        self, provided_code: str, valid_codes: List[str], user_id: str
    ) -> Tuple[bool, List[str]]:
        """
    ...
    def is_setup_complete(self, user_otp_data: Dict[str, Any]) -> bool:
        ...
        return all(
            field in user_otp_data for field in required_fields
        ) and user_otp_data.get("is_verified", False)
