import logging
from typing import TypeVar, Generic, List, Dict, Any, get_args, Optional, Tuple
...
TId = TypeVar("TId", bound=any)  # primary key type
TModel = TypeVar("TModel", bound=BaseSqlModel)  # orm model


class BaseSqlRepository(Generic[TId, TModel]):
    ...
    def get_by_id(self, id: TId) -> Optional[TModel]:
        """
    ...
    async def get_by_id_async(self, id: TId) -> Optional[TModel]:
        """
        Retrieve a single object as specified by its primary key.
        :param id: The model's primary key (e.g., 'id')
        :return: The model
    ...
    # ─── SYNCHRONOUS GET ───────────────────────────────────────────────
    def get(
        self, query_filter: Dict[str, Any] = None, skip: int = 0, limit: int = 100
    ) -> List[TModel]:
        """
    ...
    def get_paginated(
        ...
        stmt = self._build_base_paginated_get_stmt(
            query_filter, order_by, search_fields, search_text
        )
    ...
    def update(self, model: TModel, defer_commit: bool = False) -> TModel:
        ...
        # Check if the model exists by attempting to fetch it.
        if self.get_by_id(model.id):
            self._db.add(model)
            self._handle_defer_commit_single_model(model, defer_commit)
    ...
    async def update_async(self, model: TModel, defer_commit: bool = False) -> TModel:
        ...
        if await self.get_by_id_async(model.id):
            self._db.add(model)
            await self._handle_defer_commit_single_model_async(model, defer_commit)
    ...
    # ─── SYNCHRONOUS DELETE ───────────────────────────────────────────────
    def delete(self, id: TId, defer_commit: bool = False) -> None:
        """
    ...
    # ─── ASYNCHRONOUS DELETE ────────────────────────────────────────────────
    async def delete_async(self, id: TId, defer_commit: bool = False) -> None:
        """
    ...
    # ─── UTILITY METHODS ─────────────────────────────────────────────────────
    def _handle_defer_commit_single_model(
        self, model: TModel, defer_commit: bool
    ) -> None:
        if defer_commit:
            self._db.flush()
        else:
    ...
    async def _handle_defer_commit_single_model_async(
        self, model: TModel, defer_commit: bool
    ) -> None:
        if defer_commit:
        ...
        if search_text and search_fields:
            ...
            for field_name in search_fields:
                ...
                if not isinstance(column.type, (String, Text)):
                    raise TypeError(
                        f"Field '{field_name}' is not a string or text column"
                    )
    ...
    def _calculate_pagination(
        self, total: int, skip: Optional[int], limit: Optional[int]
        ...
        previous_page = (
            max((skip - limit) // limit + 1, 1) if (skip and skip > 0) else None
        )
        ...
        for part in parts:
            if hasattr(current_model, part):
                ...
                raise AttributeError(
                    f"'{current_model.__name__}' has no attribute '{part}'"
                )
        if column is None:
            raise AttributeError(
                f"Field '{field_name}' does not correspond to a valid column"
            )
        return column, joins
