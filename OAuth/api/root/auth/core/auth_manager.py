...
from auth.validators.input_validator import InputValidator
...
class AuthManager:
    ...
    async def register_user(self, user_data: UserIn, request_ip: str = None) -> TokenResponse:
        """
        ...
        await self.rate_limiter.check_limit(
            f"register:{request_ip or 'unknown'}", 
            config.RATE_LIMIT_LOGIN_PER_MIN, 
            60
        )
    ...
    async def verify_registration(self, verification_token: str, otp_code: str) -> TokenResponse:
        """
    ...
    async def login_email_password(
        ...
        # Verify password
        if not self.password_manager.verify_password(password, simulated_user["hashed_password"]):
        ...
        if config.REQUIRE_OTP and not otp_code:
            ...
            await self.email_service.send_otp_email(
                email, 
                otp_code_generated,
                simulated_user.get("first_name", "User")
            )
    ...
    async def initiate_password_reset(self, email: str, request_ip: str = None) -> Dict[str, str]:
        """
        ...
        # Rate limiting
        await self.rate_limiter.check_limit(
            f"reset:{request_ip or 'unknown'}", 
            config.RATE_LIMIT_RESET_PER_HOUR, 
            3600
        )
    ...
    async def complete_password_reset(self, reset_token: str, new_password: str) -> Dict[str, str]:
        """
    ...
    def _simulate_user_lookup(self, email: str) -> Optional[Dict[str, Any]]:
        ...
        if email == "test@example.com":
            return {
                "user_id": "user_123",
                "email": email,
                "hashed_password": self.password_manager.hash_password("TestPassword123!"),
                "roles": ["user"],
                "is_verified": True,
                "is_locked": False,
                "failed_attempts": 0,
