...
from auth.schemas import AuthFlowState, FlowStep
from auth.exceptions import InvalidFlowStateError
...
class AuthFlows:
    ...
    def start_flow(self, flow_type: AuthFlowType, user_id: str, initial_data: Dict[str, Any]) -> str:
        """
    ...
    def update_flow(self, flow_id: str, step: FlowStep, data: Dict[str, Any]) -> AuthFlowState:
        """
        ...
        if not self._is_valid_transition(flow.current_step, step):
            raise InvalidFlowStateError(f"Invalid step transition: {flow.current_step} -> {step}")
    ...
    def _is_valid_transition(self, current: FlowStep, next_step: FlowStep) -> bool:
        ...
        valid_transitions = {
            FlowStep.EMAIL_VERIFICATION: [FlowStep.OTP_VERIFICATION, FlowStep.COMPLETED],
