...
class ProviderAlreadyLinkedError(AuthServiceError):
    """Raised when attempting to link a provider that is already linked."""

    def __init__(self, message: str = "Provider already linked to account"):
        super().__init__(message, "PROVIDER_ALREADY_LINKED")


class ProviderNotLinkedError(AuthServiceError):
    """Raised when attempting to unlink a provider that is not linked."""

    def __init__(self, message: str = "Provider not linked to account"):
        super().__init__(message, "PROVIDER_NOT_LINKED")


class OTPError(AuthServiceError):
    """Raised for general OTP-related errors."""

    def __init__(self, message: str = "OTP error occurred"):
        super().__init__(message, "OTP_ERROR")
