...
from root.data.nosql.mongo.base_mongo_manager import BaseMongoManager
...
from auth_service.exceptions.auth_exceptions import InvalidTokenError, TokenExpiredError
...
class TokenManager(BaseMongoManager[str, Token]):
    ...
    def __init__(self, token_repository: TokenRepository, security_utils: SecurityUtils):
    ...
    async def generate_token_async(self, user_id: str, token_type: TokenType,
                                  additional_claims: Optional[Dict[str, Any]] = None) -> Token:
    ...
    async def revoke_user_tokens_async(self, user_id: str, token_type: Optional[TokenType] = None) -> int:
        ...
        return await self._token_repository.revoke_user_tokens_async(user_id, token_type)
    
    def get_user_tokens(self, user_id: str, token_type: Optional[TokenType] = None) -> List[Token]:
        """Get all tokens for a user."""
        return self._token_repository.find_by_user_id(user_id, token_type)
    
    async def get_user_tokens_async(self, user_id: str, token_type: Optional[TokenType] = None) -> List[Token]:
    ...
    def generate_token_pair(self, user_id: str, additional_claims: Optional[Dict[str, Any]] = None) -> Dict[str, Token]:
        """
    ...
    async def generate_token_pair_async(self, user_id: str, additional_claims: Optional[Dict[str, Any]] = None) -> Dict[str, Token]:
        """Generate both access and refresh tokens for a user (async)."""
        access_token = await self.generate_token_async(user_id, "access", additional_claims)
        refresh_token = await self.generate_token_async(user_id, "refresh", additional_claims)
