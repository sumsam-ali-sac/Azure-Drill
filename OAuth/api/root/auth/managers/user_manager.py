...
class UserManager(BaseMongoManager[str, User]):
    ...
    async def get_user_by_social_id_async(
        self, provider: str, provider_user_id: str
    ) -> Optional[User]:
        """Get user by social provider ID (async)."""
        return await self._user_repository.find_by_social_id_async(
    ...
    async def link_social_provider_async(
        self, user_id: str, provider: str, provider_user_id: str
    ) -> User:
