...
class OTPSession(BaseMongoModel[str]):
    ...
    session_token: str = Field(..., description="Unique session token for this OTP session")
    state: OTPSessionState = Field(default='pending', description="Current state of the OTP session")
    ...
    verified_at: Optional[datetime] = Field(None, description="When the OTP was successfully verified")
    ...
    @classmethod
    def create_session(cls, user_id: str, session_token: str, expiry_minutes: int = 15,
                      ip_address: Optional[str] = None, user_agent: Optional[str] = None,
                      max_attempts: int = 3) -> 'OTPSession':
