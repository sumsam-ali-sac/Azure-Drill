...
class User(BaseMongoModel[str]):
    ...
    hashed_password: Optional[str] = Field(None, description="Hashed password for email/password auth")
    social_ids: Dict[str, str] = Field(default_factory=dict, description="Social provider IDs (provider:user_id)")
    is_active: bool = Field(default=True, description="Whether the user account is active")
    first_name: Optional[str] = Field(None, description="User's first name")
    last_name: Optional[str] = Field(None, description="User's last name")
    
    # Future OTP support
    otp_secret: Optional[str] = Field(None, description="OTP secret for TOTP (future use)")
    otp_expiry: Optional[datetime] = Field(None, description="OTP expiry time (future use)")
