...
from auth.exceptions import OAuthError, InvalidStateError
from auth.security.token_utils import JWKSManager
from auth.oauth.providers import GoogleProvider, AzureProvider
...
class OAuthManager:
    ...
    def __init__(self):
        ...
        logger.info(f"OAuthManager initialized with providers: {list(self.providers.keys())}")
    ...
    async def handle_callback(
        ...
        try:
            ...
            logger.info(f"OAuth callback successful for {provider_name}: {user_info.get('email', 'unknown')}")
