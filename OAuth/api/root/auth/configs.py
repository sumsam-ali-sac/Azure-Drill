...
class AuthConfig(BaseSettings):
    ...
    JWT_ALGORITHM: str = Field(default="RS256", description="JWT algorithm (HS256, RS256, ES256)")
    ...
    JWT_PRIVATE_KEY: Optional[str] = Field(default=None, description="Private key for RS256/ES256")
    JWT_PUBLIC_KEY: Optional[str] = Field(default=None, description="Public key for RS256/ES256")
    ...
    OTP_EXPIRY_SECONDS: int = Field(default=300, description="OTP expiry time in seconds")
    MIN_PASSWORD_LENGTH: int = Field(default=12, description="Minimum password length")
    
    # Rate Limiting
    RATE_LIMIT_LOGIN_PER_MIN: int = Field(default=5, description="Login attempts per minute")
    RATE_LIMIT_OTP_PER_MIN: int = Field(default=3, description="OTP requests per minute")
    RATE_LIMIT_RESET_PER_HOUR: int = Field(default=3, description="Password reset requests per hour")
    
    # Session Configuration
    SESSION_COOKIE_NAME: str = Field(default="refresh_token", description="Session cookie name")
    ...
    def validate_google_oauth(cls, v, info):
        ...
        if (client_id and not client_secret) or (client_secret and not client_id):
            raise ValueError("Both GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET must be provided")
        return v
    ...
    def validate_azure_oauth(cls, v, info):
        ...
        if any(azure_fields) and not all(azure_fields):
            raise ValueError("All Azure OAuth fields (CLIENT_ID, CLIENT_SECRET, TENANT_ID) must be provided")
        return v
