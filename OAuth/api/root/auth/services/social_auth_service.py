...
from auth_service.exceptions.auth_exceptions import ProviderError, ValidationError
...
class SocialAuthService(BaseAuthService):
    ...
    def __init__(self, user_manager: UserManager, token_manager: TokenManager,
                 google_provider: GoogleOAuthProvider, azure_provider: AzureOAuthProvider):
    ...
    async def authenticate(self, credentials: Dict[str, Any], set_cookies: bool = False) -> Union[Dict[str, Any], Response]:
        """
        ...
        try:
            ...
            if id_token:
                try:
                    ...
                    id_token_payload = self._security_utils.verify_provider_id_token(id_token, provider_name)
                    user_info = self._extract_user_info_from_id_token(provider_name, id_token_payload)
            ...
            raise ProviderError(f"Social authentication failed: {str(e)}", provider_name)
    
    def _extract_user_info_from_id_token(self, provider_name: str, id_token_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if provider_name.lower() == 'google':
            ...
            return {
                ...
                "email": id_token_payload.get("email") or id_token_payload.get("preferred_username"),
                "first_name": id_token_payload.get("given_name"),
                "last_name": id_token_payload.get("family_name"),
                "name": id_token_payload.get("name"),
    ...
    def register(self, user_data: Dict[str, Any]) -> User:
        ...
        """
        raise NotImplementedError("Social registration is handled automatically during authentication")
    ...
    def link_provider(self, user_id: str, provider_name: str, auth_code: str, state: Optional[str] = None) -> User:
        """
        ...
        try:
            ...
            existing_user = self._user_manager.get_user_by_social_id(provider_name, user_info["id"])
            if existing_user and existing_user.id != user_id:
                raise ProviderError(f"This {provider_name} account is already linked to another user", provider_name)
            
            # Link provider to user
            user = self._user_manager.link_social_provider(user_id, provider_name, user_info["id"])
            ...
            if isinstance(e, ProviderError):
                raise
            raise ProviderError(f"Failed to link {provider_name} account: {str(e)}", provider_name)
    
    async def link_provider_async(self, user_id: str, provider_name: str, auth_code: str, state: Optional[str] = None) -> User:
        ...
        try:
            ...
            # Check if this social account is already linked to another user
            existing_user = await self._user_manager.get_user_by_social_id_async(provider_name, user_info["id"])
            if existing_user and existing_user.id != user_id:
                raise ProviderError(f"This {provider_name} account is already linked to another user", provider_name)
            
            # Link provider to user
            user = await self._user_manager.link_social_provider_async(user_id, provider_name, user_info["id"])
    ...
    async def unlink_provider_async(self, user_id: str, provider_name: str) -> User:
        ...
        return await self._user_manager.unlink_social_provider_async(user_id, provider_name)
    ...
    def _find_or_create_user(self, provider_name: str, user_info: Dict[str, Any]) -> User:
        ...
        if email:
            try:
                ...
                user = self._user_manager.link_social_provider(user.id, provider_name, provider_user_id)
                return user
            except:
    ...
    async def _find_or_create_user_async(self, provider_name: str, user_info: Dict[str, Any]) -> User:
        """Find existing user or create new one based on social provider info (async)."""
        provider_user_id = user_info["id"]
        email = user_info.get("email")
        
        # First, try to find user by social provider ID
        user = await self._user_manager.get_user_by_social_id_async(provider_name, provider_user_id)
        if user:
            return user
        ...
        if email:
            try:
                ...
                # Link this social provider to the existing user
                user = await self._user_manager.link_social_provider_async(user.id, provider_name, provider_user_id)
                return user
            except:
                # User with this email doesn't exist, create new one
                pass
