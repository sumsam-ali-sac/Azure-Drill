...
from auth.exceptions import AuthConfigurationError
...
class SecretManager:
    ...
    def __init__(self):
        ...
        self._private_key: Optional[Union[rsa.RSAPrivateKey, ec.EllipticCurvePrivateKey]] = None
        self._public_key: Optional[Union[rsa.RSAPublicKey, ec.EllipticCurvePublicKey]] = None
    ...
    def get_private_key(self) -> Union[rsa.RSAPrivateKey, ec.EllipticCurvePrivateKey]:
        ...
        if self._private_key is None:
            ...
            if not private_key_data:
                raise AuthConfigurationError("Private key not configured for asymmetric algorithm")
        ...
        if self._public_key is None:
            ...
            if not public_key_data:
                ...
                try:
                    ...
                    raise AuthConfigurationError("Public key not configured and cannot derive from private key")
    ...
    def get_oauth_secret(self, provider: str) -> str:
        ...
        if not secret:
            raise AuthConfigurationError(f"OAuth secret not configured for provider: {provider}")
    ...
    def get_oauth_client_id(self, provider: str) -> str:
        ...
        if not client_id:
            raise AuthConfigurationError(f"OAuth client ID not configured for provider: {provider}")
