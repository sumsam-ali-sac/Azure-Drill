...
class OTPVerifySetupRequest(BaseModel):
    ...
    otp_code: str = Field(
        ..., description="OTP code from authenticator app", min_length=6, max_length=6
    )
...
class OTPVerifyRequest(BaseModel):
    ...
    otp_code: str = Field(
        ..., description="OTP code to verify", min_length=6, max_length=8
    )
...
class OTPAuthRequest(BaseModel):
    ...
    otp_code: str = Field(
        ..., description="OTP code for authentication", min_length=6, max_length=8
    )
...
async def setup_otp(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="OTP setup failed"
        )
...
async def verify_otp_setup(
    ...
    """
    try:
        success = await otp_service.verify_totp_setup_async(
            current_user.id, request.otp_code
        )
        if success:
            return SuccessResponse(
                success=True, message="OTP setup verified successfully"
            )
        else:
            raise HTTPException(
...
async def authenticate_with_otp(
    ...
    """
    try:
        credentials = {"user_id": request.user_id, "otp_code": request.otp_code}
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="OTP authentication failed",
        )
...
async def disable_otp(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Failed to disable OTP"
            )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def generate_recovery_codes(
    ...
    """
    try:
        recovery_codes = await otp_service.generate_recovery_codes_async(
            current_user.id
        )
