...
from auth_service.api.dependencies import get_social_auth_service, get_current_user
...
async def get_supported_providers(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to get providers")
...
async def get_auth_url(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to get authorization URL")
...
async def authenticate_social(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Social authentication failed")
...
async def link_provider(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to link provider")
...
async def unlink_provider(
    ...
    """
    try:
        user = await social_service.unlink_provider_async(current_user.id, request.provider)
        return UserResponse(user=user.dict())
        
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to unlink provider")
...
async def azure_callback(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Azure authentication failed")
