...
class BaseManager(Generic[TModel], ABC):
    """
    Abstract base class for managing "business" or "domain" level logic for a domain model.
    ...
    async def create_async(self, model: TModel, defer_commit: bool = False) -> TModel:
        ...
        :rtype: TModel
        """
        return await self._repository.create_async(
            model=model, defer_commit=defer_commit
        )
    ...
    async def get_async(
        ...
        :return: List of models
        """
        return await self._repository.get_async(
            query_filter=query_filter, skip=skip, limit=limit
        )
