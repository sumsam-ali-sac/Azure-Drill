...
class AuthUser:
    ...
    def has_permission(
        ...
        return rbac_manager.user_has_permission(
...
def require_permission(resource: Union[ResourceType, str], action: Union[Action, str]):
    """
    ...
    async def permission_dependency(
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail=str(e.detail)
            )
...
def permission_required(resource: Union[ResourceType, str], action: Union[Action, str]):
    """
...
def create_error_response(
    error: Exception, status_code: int = status.HTTP_400_BAD_REQUEST
) -> JSONResponse:
    """
